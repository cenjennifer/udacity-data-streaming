...
customerRiskKafkaMessageSchema = StructType (
    [
...
spark = SparkSession.builder.appName("customer-stedi-events").getOrCreate()
...
zSetDecodedEntriesStreamingDF1= zSetEntriesEncodedStreamingDF.withColumn("encodedCustomer", unbase64(zSetEntriesEncodedStreamingDF.encodedCustomer).cast("string"))
...
# TO-DO: cast the value column in the streaming dataframe as a STRING 
kafkaServerRawStreamingDF = kafkaServerRawStreamingDF.selectExpr("cast(value as string) value")
...
kafkaServerRawStreamingDF.withColumn("value",from_json("value",customerRiskKafkaMessageSchema))\
        .select(col('value.*')) \
...
customerRiskStreamingDF = emailAndBirthYearStreamingDF.join(customerRiskStreamingDF, expr("""
    customer = email
"""))
...
customerRiskStreamingDF.selectExpr("cast(customer as string) as key", "to_json(struct(*)) as value") \
    ...
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092")\
    .option("topic", "customer-stedi-risk")\
